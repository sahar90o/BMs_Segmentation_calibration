...
def convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits: Union[torch.Tensor, np.ndarray],
    ...
    if return_probabilities:
        ...
        predicted_logits = label_manager.revert_cropping_on_probabilities(predicted_logits, properties_dict['bbox_used_for_cropping'], properties_dict['shape_before_cropping'])
        predicted_logits = predicted_logits.transpose([0]+[i+1 for i in plans_manager.transpose_backward])
...
def export_prediction_from_logits(predicted_array_or_file: Union[np.ndarray, torch.Tensor], properties_dict: dict,
    ...
    if save_probabilities:
        segmentation_final, probabilities_final, logits = ret
        np.savez_compressed(output_file_truncated + '.npz', probabilities=probabilities_final)
        np.savez_compressed(output_file_truncated.replace(".nii.gz", "") + '_logits.npz', logits=logits)
