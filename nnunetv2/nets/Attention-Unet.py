...
train_loader = DataLoader(train_ds, batch_size=1, shuffle=True, num_workers=30)

val_ds = CacheDataset(data=val_files, transform=val_transforms, cache_rate=0.5, num_workers=30)
val_loader = DataLoader(val_ds, batch_size=1, num_workers=30)
...
#loss_function = FocalLoss(gamma=2.0, to_onehot_y=True, use_softmax=True)
    ...
    if (epoch+1) % val_interval ==0:
        ...
        with torch.no_grad():
            ...
            if metric > best_metric:
                ...
                torch.save(model.state_dict(), os.path.join(root_dir,"Attention-Unet-Dice-focal.pth"))
